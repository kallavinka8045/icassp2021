...
In this paper, we propose Universal MelGAN: a robust neural vocoder for high-fidelity speech synthesis. The proposed model has the following improvements based on the MelGAN architecture. 1) In order to generate waveforms of multiple  domains, all hidden channel sizes were increased by 4 times and the gated activation unit (GAU) was added to the last layer of each residual stack. 2) Multi-resolution spectrogram discriminators are connected to the model to  alleviate the over-smoothing problem in the high frequency band caused by the previous improvement. As a result, our model trained by hundreds of speakers recorded the best MOS score among the competing baselines in most seen and unseen domain scenarios such as speaker, language, recording condition or emotion in both Korean and English. In multi-speaker text-to-speech, high-fidelity waveforms were generated regardless of whether seen or unseen(a real-time synthesis speed of 0.028 RTF on NVIDIA V100 GPU). This result without any explicit domain information suggests the possibility of the proposed model as a universal vocoder.
